...
export default function CourseInfoImage({value,introImgList,setIntroImgList,accessToken,setDummyImgUrlUpdate}){
    ...
    const uploadImg = (e) => {
        uploadImageApi(e.target.files[0],accessToken).then(({data})=>{setImgValue({file:e.target.files[0],url:data.originalFilePath});}).catch((err)=>{console.log(err)})
    };
